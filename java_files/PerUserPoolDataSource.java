...
public class PerUserPoolDataSource extends InstanceKeyDataSource
{
    ...
    public int getNumActive(final String username, final String password) {
        final ObjectPool pool = (ObjectPool)this.getPool(this.getPoolKey(username, password));
    ...
    protected PooledConnectionAndInfo getPooledConnectionAndInfo(final String username, final String password) throws SQLException {
        ...
        synchronized (this) {
            ...
            if (manager == null) {
                try {
                    ...
                    throw new SQLNestedException("RegisterPool failed", (Throwable)e);
                }
            }
        ...
        try {
            ...
            throw new SQLNestedException("Could not retrieve connection info from pool", (Throwable)ex);
        }
        catch (Exception e2) {
            try {
            ...
            try {
                this.registerPool(username, password);
                pool = (ObjectPool)this.getPool(key);
            }
            catch (NamingException ne) {
                throw new SQLNestedException("RegisterPool failed", (Throwable)ne);
            }
            try {
                info = (PooledConnectionAndInfo)pool.borrowObject();
            }
    ...
    private synchronized void registerPool(final String username, final String password) throws NamingException, SQLException {
        ...
        final GenericObjectPool pool = new GenericObjectPool((PoolableObjectFactory)null);
        ...
        final CPDSConnectionFactory factory = new CPDSConnectionFactory(cpds, (ObjectPool)pool, this.getValidationQuery(), this.isRollbackAfterValidation(), username, password);
    ...
    private void readObject(final ObjectInputStream in) throws IOException, ClassNotFoundException {
        try {
            ...
            final PerUserPoolDataSource oldDS = (PerUserPoolDataSource)new PerUserPoolDataSourceFactory().getObjectInstance((Object)this.getReference(), (Name)null, (Context)null, (Hashtable)null);
    ...
    public static void XXXmyClinitXXX() {
        Clinit.logClinit("org/apache/commons/dbcp/datasources/PerUserPoolDataSource");
    }
}
