...
public class PeriodAxis extends ValueAxis implements Cloneable, PublicCloneable, Serializable
{
    ...
    public PeriodAxis(final String label, final RegularTimePeriod first, final RegularTimePeriod last, final TimeZone timeZone) {
        super(label, (TickUnitSource)null);
        ...
        (this.labelInfo = new PeriodAxisLabelInfo[2])[0] = new PeriodAxisLabelInfo(Month.class, (DateFormat)new SimpleDateFormat("MMM"));
        this.labelInfo[1] = new PeriodAxisLabelInfo(Year.class, (DateFormat)new SimpleDateFormat("yyyy"));
    }
    ...
    protected void drawTickMarksHorizontal(final Graphics2D g2, final AxisState state, final Rectangle2D dataArea, final RectangleEdge edge) {
        ...
        for (long lastOnAxis = this.getLast().getLastMillisecond(this.getTimeZone()); t2 <= lastOnAxis; t2 = t.getFirstMillisecond(this.getTimeZone())) {
            ticks.add(new NumberTick((Number)new Double((double)t2), "", TextAnchor.CENTER, TextAnchor.CENTER, 0.0));
    ...
    public static void XXXmyClinitXXX() {
        Clinit.logClinit("org/jfree/chart/axis/PeriodAxis");
        PeriodAxis.class$org$jfree$data$time$Month = null;
        PeriodAxis.class$org$jfree$data$time$Year = null;
        PeriodAxis.class$java$util$Date = null;
        PeriodAxis.class$java$util$TimeZone = null;
    }
}
